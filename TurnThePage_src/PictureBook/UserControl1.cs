...
namespace PictureBook
{
    public partial class TurnThePageControl : UserControl
    {
        ...
        // in the animation
        private const bool INCLUDE_SPIRAL_SPINE = true;
        ...
        private  int PAGE_WIDTH;   // Width of the page
        private  int PAGE_HEIGHT;  // Height of the page

        //Sample Bitmaps, used to make the pages of this Picture Book
        private Bitmap[] bitmaps; // Container for all pages in this book

        // Combined offscreen image
        private Bitmap CurrentShownBitmap=null;
        ...
        private int currentLeftPage = 3;
        private int currentRightPage = 4;

        private int animatingToLeftPage;
        private int animatingToRightPage;
        ...
        public TurnThePageControl()
        {
            ...
            Stream stream =
              assem.GetManifestResourceStream(
                 "PictureBook.Resources.SingleBinderLeft.bmp"
              );
            ...
            stream =
              assem.GetManifestResourceStream(
                 "PictureBook.Resources.SingleBinderRight.bmp"
              );
...
//                    outAngle = previous_a;
        ...
        // Loads the sample images.
        public void LoadSamples()
        {
            // Sets up an array of colors. This will give each page a different color.
            Color[] myColors = new Color[] { Color.Blue,Color.Green,Color.Aquamarine,Color.Red,Color.Yellow,Color.Orange};
            bitmaps = new Bitmap[6];

            // Prepare the off screen combined bitmap.
            CurrentShownBitmap = new Bitmap(this.ClientRectangle.Width, this.ClientRectangle.Height);

            // The hotSpot is the location of the line of symmetry.
            hotSpot = new Circle(new Point(230, 370), 6);

            //  Create 6 Bitmaps for this animation
 
            // These fields are used to initialize and bound the movement of the hotspot.
            PAGE_SPINE = this.Width / 2; // This should be the middle
            PAGE_BOTTOM = this.Height; // Location of the bottom of the pages. Height from the top of the control
            BOOK_WIDTH = this.Width; // Edge of the page
            PAGE_WIDTH = this.Width / 2;
            PAGE_HEIGHT = this.Height - heightOffsetForAnimationTop;

            // Create a linear gradient brush with alpha for the page Spine
            LinearGradientBrush lgb = new LinearGradientBrush((new Rectangle(0, 0, 2, PAGE_HEIGHT)),
                (Color.FromArgb(80, 80, 80, 80)), Color.FromArgb(80, Color.White), LinearGradientMode.Horizontal);
            

            for (int i = 1; i <= 6; i++)
            {
                bitmaps[i - 1] = new Bitmap(this.Width / 2, this.Height - heightOffsetForAnimationTop);
                drawPageBitmap(bitmaps[i - 1], (i % 2 == 0), myColors[i - 1], i, INCLUDE_SPIRAL_SPINE, lgb);

            }                        
        }


        // Used to draw all the pages bitmaps. This is used during initialization and called only by
        // LoadSamples
        private void drawPageBitmap(Image bitmap,bool isRight,Color c, int number,bool includeSpiral,LinearGradientBrush lgb)
        {
            Graphics g;
            g = Graphics.FromImage(bitmap);
            g.SmoothingMode = SmoothingMode.AntiAlias;
            g.FillRectangle(new SolidBrush(c), new Rectangle(0, 0, bitmap.Width, bitmap.Height));
            g.DrawString(number.ToString(), new Font("Arial", 180), new SolidBrush(Color.Black), new PointF(-10F, 10F));
            if (INCLUDE_SPIRAL_SPINE == true)
            {
                if(isRight)
                {
                    DrawRightPageSpine(g, PAGE_SPINE, bitmap.Width, bitmap.Height);

                }
                else
                {
                    DrawLeftPageSpine(g, PAGE_SPINE,bitmap.Width, bitmap.Height);


                }
            }
            else // Do not use a Spiral Spine
            {
                if(isRight)
                {
                    g.FillRectangle(lgb, 0, 0, 3, bitmap.Height);
                }
                else
                {
                    g.FillRectangle(lgb, bitmap.Width - 3, 0, 2, bitmap.Height);
                }
            }
            g.DrawRectangle(new Pen(Brushes.Black, 2f), new Rectangle(0, 0, bitmap.Width, bitmap.Height));
            g.Dispose();           
        ...
        {
            // Fill in the offscreen bitmap.
            if (CurrentShownBitmap != null)
            {
                this.BackColor = this.Parent.BackColor;
                Graphics g = Graphics.FromImage(CurrentShownBitmap);
                g.Clear(this.BackColor);
                // Show page 3 and page 4. Start in the middle
                g.DrawImage(bitmaps[2], new Point(0, heightOffsetForAnimationTop));
                g.DrawImage(bitmaps[3], new Point(bitmaps[2].Width, heightOffsetForAnimationTop));
                g.Dispose();
            }
       ...
       // backside of the turning page.  It will return either a triangle or trapezoid graphics path.
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            if (CurrentShownBitmap != null)
            {
                e.Graphics.DrawImage(CurrentShownBitmap, new Point(0, 0));
            }
        ...
        {
            if (isInAnimation == false && currentLeftPage>1)
           {
               ...
               timer1.Enabled = true;
               animatingToLeftPage = currentLeftPage - 2;
               animatingToRightPage = currentRightPage - 2;
        ...
        {
            if (isInAnimation == false && currentLeftPage<5)
            {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (currentXdistanceFromEdge >= (PAGE_SPINE))
            {
                ...
                currentLeftPage = animatingToLeftPage;
                currentRightPage = animatingToRightPage;

                // At this point the animation is done, draw in the new bitmaps fully
                g.DrawImage(bitmaps[currentLeftPage - 1], new Point(0, heightOffsetForAnimationTop));
                g.DrawImage(bitmaps[currentRightPage - 1], new Point(bitmaps[currentLeftPage - 1].Width, heightOffsetForAnimationTop));
